...
for user in data:
    ...
    print(temporary)           #если добавить длэтой и 2х последующих строчек отступ, список отображается с каждой итерацией цикла for a in value:
    temp_length=len(temporary) #если оставить так, то программа игнорирует эту часть и сразу переходит к следующему человеку, меняя ему время на питерское
    for i in temporary:
        print(i)
        break
        '''ii=0
